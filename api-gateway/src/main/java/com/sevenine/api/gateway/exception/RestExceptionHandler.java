...
public class RestExceptionHandler {
    ...
    @ExceptionHandler(ApiConnectaException.class)
    public ResponseEntity<?> handleResourceNotFoundException(ApiConnectaException api) {
        ErrorDetail errorDetail = new ErrorDetail();
        errorDetail.setTimeStamp(new Date().getTime());
        errorDetail.setStatus(api.getStatus().value());
        errorDetail.setTitle("Erro ao processar");
        errorDetail.setDetail(api.getMensagem());
